...
#--        与数据库交互放到服务端
            ...
            print('第223行respon为:',respon)
            print('第224行respon.text为：',respon.text)
            result = eval(respon.text)
            print('第225行result为:',type(result))
            if len(result)==0:
...
# 此类中重写createwindow內建函数可以让窗口中的超链接有反应
...
#1@@
        self.register_name_text.installEventFilter(self)


        #帐号要求
        self.register_name_yq = QLabel('* 昵称：字母、数字、英文下划线',self)
#@@
        self.register_name_yq.move(235,102)
        self.register_name_yq.setStyleSheet('QLabel{color:white}' 'QLabel{font-family:"华文新魏"}')

#2@@        
        self.register_name_yq2 = QLabel('（不超过8位）',self)
        self.register_name_yq2.move(280,122)
        self.register_name_yq2.setStyleSheet('QLabel{color:white}' 'QLabel{font-family:"华文新魏"}')
#@@
        #密码输入框
        ...
        # 账号框过滤
#3@@
        reg_name = QRegExp('^\w{1,8}$')
        pValidator = QRegExpValidator(self)
        pValidator.setRegExp(reg_name)
        self.register_name_text.setValidator(pValidator)
#@@
...
#--
        ...
        #邮箱框过滤
        reg_email = QRegExp('^[\w.-]+@{1}[A-Za-z\d-]+\.{0,1}[A-Za-z\d-]+\.{1}[A-Za-z]{2,8}$')
...
#4@@
        ...
        pwd = self.register_pwd_text.text()
        pwd2 = self.register_pwd2_text.text()
        if name == '' or email == '' or pwd == '' or pwd2 == '':
            QMessageBox.warning(self,'高德API在线租房系统','表单不能为空',QMessageBox.Ok,QMessageBox.Ok)

        elif pwd2 != pwd:
            QMessageBox.warning(self,'高德API在线租房系统','密码两次输入不一致,请重新输入',QMessageBox.Ok,QMessageBox.Ok)
        elif not re.findall('^[\w.-]+@{1}[A-Za-z0-9-]+\.{0,1}[A-Za-z0-9-]+\.{1}[A-Za-z]{2,8}$',email):
            self.register_email_text.setText('')
            QMessageBox.warning(self,'高德API在线租房系统','邮箱地址无效！',QMessageBox.Ok,QMessageBox.Ok)
...
#5@@
                self.register_name_text.setText('')
#@@
